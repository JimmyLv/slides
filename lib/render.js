...
const md = require('../plugin/markdown/markdown');
...
function parseSlides(markdown, extraOptions) {
  ...
  const parsedMarkdown = yaml.markdown
    .replace(/\`\`\`(\w+)/g, `<figure class="code"><pre>
\`\`\`$1`)
  return view.preprocessorFn(parsedMarkdown, view).then(processedMarkdown => {
